...
class RoundedButton(Widget, _MixinTextualWidget):
    """The Button widget, rounded version without a shadow.
    Has to be used in conjunction with its event onclick.
    Use Widget.onclick.connect in order to register the listener.
    """
    def __init__(self, text='', *args, **kwargs):
        """
        Args:
            ...
            kwargs: See Widget.__init__()
        """
        style = {**kwargs.pop("style", {}), "box-shadow": "none", "border-radius": "10px", "border-style": "solid"}
        kwargs["style"] = style
...
class Spinner(Widget):
    ...
    def __init__(self, size: int = 32, color: str = "#000", thickness: int = 3, *args, **kwargs):
        """Initialize the spinner

        Args:
            size (int, optional): Size of spinner. Defaults to 32.
            color (str, optional): Color of spinner. Defaults to "#000".
            kwargs: See Widget.__init__()
        """
        style = {
            "width": f"{size}px",
            "height": f"{size}px",
            # Feeble attempt to center the spinner vertically when it appears next to a button
            "margin": "10px 10px -5px 10px",
            "border": f"{thickness}px solid {color}",
            "border-bottom-color": "transparent",
            "border-radius": "50%",
            "display": "inline-block",
            "box-sizing": "border-box",
            "animation": "rotation 1s linear infinite",
            "visibility": "hidden", # Spinner is hidden when created
        }
    ...
    def stop(self):
        # TODO: This might cause layout issues.
